// useAiSummary composableのテスト。
// $fetch (Nuxt auto-import) をvi.stubGlobalで差し替えて、成功 / エラー / 多重呼び出しを検証する。
// サーバー側createError({ data: { error: SummaryErrorCode } }) を
// labelForSummaryErrorで日本語ラベルに変換してerror.valueにセットする方式。
...
describe('useAiSummary', () => {
  ...
  it('初期状態はsummary=null / loading=false / error=null', () => {
  ...
  it('成功時にsummaryを更新しloadingがfalseに戻る', async () => {
  ...
  it('未知のcodeはunknownラベルにフォールバック', async () => {
  ...
  it('loading中の再呼び出しは無視する (二重呼び出し防止)', async () => {
  ...
  it('resetでsummaryとerrorをクリアする', async () => {
