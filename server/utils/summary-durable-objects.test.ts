...
describe('SummaryCacheDO', () => {
  it('claim後の同一cacheKeyはpendingになり、store後にhitになる', async () => {
  ...
  it('期限切れcacheとpendingはmissとして扱う', async () => {
    const cache = new SummaryCacheDO(makeState());
    await cache.store({
...
describe('GlobalSummaryQuotaDO', () => {
  it('quotaはreserve時点で消費し、上限到達後はretryAfterSecondsを返す', async () => {
  ...
  it('成功とupstream call後失敗はreserve countと別に記録できる', async () => {
