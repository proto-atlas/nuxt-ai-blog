...
describe('evaluateSummaryQuality', () => {
  ...
  it('必須語がsourceとsummaryにあり、文字数と禁止語条件を満たすとpassになる', () => {
    const result = evaluateSummaryQuality({
      ...
      sourceText:
        'Nuxt 4 をCloudflare Workersにdeployする。Nitroのcloudflare_module preset、wrangler、Secrets、D1 切替を扱う。',
      summary:
  ...
  it('summaryが長すぎるとwithin-lengthだけfailになる', () => {
    const result = evaluateSummaryQuality({
      ...
      sourceText: 'Tailwind CSS 4 はCSS-first設定、カスタムバリアント、ビルド速度改善を扱う。',
      summary:
  ...
  it('本文外の禁止語やsecret-like textを含むとfailになる', () => {
    const result = evaluateSummaryQuality({
      ...
      sourceText:
        'TypeScript 6 とVue 3.5 でcomposableを書くときの型付け、戻り値、副作用管理を扱う。',
