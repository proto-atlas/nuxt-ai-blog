...
/**
 * AI要約のcacheとin-flight状態をDurable Object Storageに保存する。
 *
 * in-memoryのpending signalは同一Object lifetime内の待機効率化だけに使い、
 * eviction / restart後も判断できるpending markerとsummaryはstorage側に置く。
 */
export class SummaryCacheDO {
  ...
  async claim(input: SummaryCacheClaimInput): Promise<SummaryCacheClaimResult> {
    // 同時requestが先にcacheを埋めた可能性があるため、claim直前にも再確認する。
...
/**
 * 日次 実API生成quotaを固定名Durable Objectに集約する。
 *
 * quotaは「実API呼び出しを開始する権利」としてreserveするため、
 * upstream call開始後の失敗もcost exposureとしてreserved countに含める。
 */
