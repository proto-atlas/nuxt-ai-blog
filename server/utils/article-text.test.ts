...
describe('buildSummarySource', () => {
  it('title + description + bodyのtext nodeを改行で結合する', () => {
    const article = {
      title: 'タイトル',
      description: '概要文',
      body: {
  ...
  it('bodyがASTでなくてもtitle + descriptionで要約源を作れる (fallback)', () => {
    const article = { title: 'タイトル', description: '概要文', body: null };
    expect(buildSummarySource(article)).toBe('タイトル\n\n概要文');
  });

  it('titleが空でもdescription + bodyだけで作れる', () => {
    const article = {
      title: '',
      description: '概要文',
  ...
  it('本文がMAX_BODY_TEXT_LENGTHを超えたらtruncateする', () => {
