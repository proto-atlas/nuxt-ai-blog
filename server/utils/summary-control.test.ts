...
describe('summary-control', () => {
  ...
  it('同じslug / model / sourceTextなら同じcache keyを返す', async () => {
  ...
  it('sourceTextが変わるとarticle hashが変わりcache keyも変わる', async () => {
    const first = await buildSummaryCacheKey({
      slug: 'nuxt-content-workers',
      model: 'claude-haiku-4-5-20251001',
  ...
  it('Cloudflare bindingが揃っていればdurable controlを返す', () => {
    const quotaBinding = {
  ...
  it('bindingが無い場合はmissingBindingsとmemory fallbackを返す', () => {
  ...
  it('production判定はNODE_ENV=productionのときだけtrue', () => {
  ...
  it('memory fallbackはclaim中の同一keyをpendingとして扱いrelease後にtimeoutへ戻る', async () => {
    const result = resolveSummaryControl({});
  ...
  it('memory fallbackはstore後にcache hitを返す', async () => {
    const result = resolveSummaryControl({});
