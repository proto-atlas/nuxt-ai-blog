...
- **30 秒で見る**: Public demo で記事一覧、検索、タグ絞り込み、記事詳細をアクセスキーなしで確認できます。
...
Anthropic API は従量課金のため、無認証で公開すると AI コストの消費攻撃を受ける可能性があります。本デモのアクセスキーはユーザー認証ではなく、公開ポートフォリオの live AI 要約を守る cost guard です。live AI 機能は、アクセスキー + per-IP short-window guard + 固定名 `GlobalSummaryQuotaDO` の global daily quota + `SummaryCacheDO` + Anthropic Spend Limit の多層防御で守っています。本番 SaaS として運用する場合は、相手別キー、期限付きキー、利用量の相手別追跡を追加する想定です。
...
| Vitest unit | `npm run test:coverage -- --maxWorkers=1` | **126 件 pass / 20 ファイル** |
| Vitest coverage | `npm run test:coverage -- --maxWorkers=1` | stmts 85.51 / branches 79.67 / funcs 90.74 / lines 87.10 (gate `lines>=60 / branches>=50 / funcs>=70 / statements>=60` 通過) |
...
| Quality gate | `npm run verify:portfolio` | 上記の主要検査を順に実行 |
...
| [`docs/evidence/release-baseline-2026-04-29.md`](docs/evidence/release-baseline-2026-04-29.md) | release verification baseline |
...
- Vitest 4 + @nuxt/test-utils + happy-dom (Unit 126 件、coverage stmts 85.51 / branches 79.67 / funcs 90.74 / lines 87.10)
...
| ジョブ | 内容 |
|---|---|
| `quality-gate` | typecheck → lint → `test:coverage` (閾値強制) → `npm audit --audit-level=high` → secret scan → build |
...
dev / Node.js native sqlite で動かしており、Cloudflare Workers 本番では `@nuxt/content` が D1 database に自動切替する旨の警告が出ます (`Deploying to Cloudflare requires using D1 database`)。SPA 遷移時の Nuxt Content client query が sqlite-wasm を使うため、CSP は `unsafe-eval` を許可せず WebAssembly 限定の `wasm-unsafe-eval` だけを `script-src` に追加しています。D1 binding への完全移行と client bundle の sqlite-wasm 削減は今後の運用課題です。
