...
`docs/evidence/` 配下に各 gate の検証根拠を保存。production smoke / `npm audit` は本番環境での再計測時に evidence を追加する。
...
- **セキュリティヘッダ 6 件**: nosniff / X-Frame-Options DENY / Referrer-Policy / Permissions-Policy / HSTS / Content-Security-Policy を Nitro `routeRules` で全ルート一括付与。`X-Powered-By: Nuxt` も同じ仕組みで空文字上書きして抑止
