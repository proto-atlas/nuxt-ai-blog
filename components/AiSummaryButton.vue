...
/**
 * AI要約ボタン + 結果表示。
 * slugをpropで受け取り、クリックでuseAiSummary.summarize(slug) を呼ぶ。
 * 表示段階: 初期 (ボタンのみ) → 取得中 (spinner) → 結果表示 (summary + cachedバッジ)
 */
...
<template>
  ...
  >
    <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
      <div>
        <h3 class="text-sm font-semibold text-emerald-900 dark:text-emerald-100">AI要約</h3>
      ...
      <div class="flex w-full flex-col gap-2 sm:w-auto sm:min-w-72">
        ...
        <div class="flex flex-col gap-2 sm:flex-row">
          ...
          <button
            ...
            @click="handleClick"
          >
            {{ loading ? '生成中...' : summary ? '再生成' : 'AI要約を生成' }}
          </button>
        </div>
      </div>
    </div>
    ...
    <p v-else class="mt-3 text-xs leading-relaxed text-emerald-900 dark:text-emerald-200">
      実API要約は公開URLのコスト保護のため、アクセスキー入力後に実行できます。
      記事本文、検索、タグ、記事詳細はアクセスキーなしで確認できます。
    </p>
  </div>
</template>
