...
// AiSummaryButton.vueのテスト。
// useAiSummaryはNuxt auto-importなのでmockNuxtImportで差し替える。
...
describe('AiSummaryButton', () => {
  ...
  it('summaryが入っていれば本文とmodelを表示する', async () => {
    aiSummaryStub = makeStub({
      summary: {
        slug: 'sample',
  ...
  it('cached=trueで「キャッシュ」バッジを表示する', async () => {
    aiSummaryStub = makeStub({
      summary: {
        slug: 'sample',
        summary: 'cachedテキスト',
