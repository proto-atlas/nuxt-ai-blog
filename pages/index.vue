...
/**
 * 記事一覧ページ。
 * queryCollection('blog') でNuxt Content 3 のblogコレクションを取得し、
 * date DESCでソートしてArticleCardで表示する。
...
const description =
  '技術ブログの記事一覧。Nuxt / TypeScript / Cloudflare / Tailwind / ESLint等のトピック。';
...
useHead({
  ...
  // 同じページが複数URLでアクセス可能な場合 (trailing slash等) でも検索評価が分散しない。
  link: [{ rel: 'canonical', href: canonicalUrl }],
});

// OG / Twitter cardのページ個別項目。app.vueでog:type / og:site_nameは設定済み。
useSeoMeta({
  ...
  <div>
    <div class="mb-8">
      ...
      <p class="mt-2 text-sm text-slate-600 dark:text-slate-400">
        Nuxt / Vue / TypeScript / Cloudflare Workers周辺の技術ノート。AI要約は記事詳細ページから。
      </p>
    </div>
