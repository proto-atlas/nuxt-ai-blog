 ...
 * Nuxt Content 3 はfrontmatter + Markdownからpathを自動生成 (/blog/<slug>) するため、
 * route.pathを使ってクエリするだけで該当記事を取れる。
 */
const route = useRoute();

const slug = route.params.slug as string;

// 一覧 → 詳細のSPA遷移時に `queryCollection('blog').path('/blog/<slug>').first()`
// がdev環境でnullを返し 404 ページに飛ぶ事象が出ていた (SSR直接アクセスはOK)。
// 原因はNuxt Content 3 のSQL BuilderがSPA遷移時のclient側fetchでpath
// 完全一致を取りこぼすケースで、回避策として全コレクション (5 件程度) を取って
// JS側でfilterする。記事数が常に小規模でoverheadが無視できる前提。
...
useHead(() => ({
  ...
  // 検索エンジン向けに重複コンテンツを統一するcanonical URL。
  link: [{ rel: 'canonical', href: canonicalUrl.value }],
}));

// OG / Twitter cardの記事個別項目。og:typeは記事ページなのでarticleで上書き。
// app.vue側のwebsite設定より優先される (NuxtのuseSeoMetaは後勝ち)。
useSeoMeta({
  ogType: 'article',
