...
注意: このsnapshotは Durable Objects summary cache / global daily quota 実装前の本番smokeです。DO実装後の `cached:true` と quotaRemaining は、deploy後に manual-live-summary-smoke として別途記録します。
...
- Durable Objects summary cache / global daily quota 実装後の本番 `cached:true` は、このsnapshotでは未確認。deploy後に既知cold cache keyで manual-live-summary-smoke を再実行する。
