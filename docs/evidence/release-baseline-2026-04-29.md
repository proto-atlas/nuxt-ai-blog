...
この証跡は特定時点の検証スナップショットであり、リポジトリの最新HEADであることは主張しません。固定HEADは本文に埋め込まず、第三者確認で対象にするcommitとCI runは外部入力として指定してください。
...
| GitHub Actions | `gh run list --repo proto-atlas/nuxt-ai-blog --limit 3` | completed run success at verification time |
...
- このsnapshot時点では本番cache hitは未確認だった。Durable Objects cache / quota 実装後の本番 `cached:true` は、deploy後の manual-live-summary-smoke で別途記録する。
